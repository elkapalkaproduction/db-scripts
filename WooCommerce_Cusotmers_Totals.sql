...
FROM 
(Select pm.post_id as order_id, pm.meta_value as user_id from wp_postmeta pm
where pm.post_id in
(SELECT pm.post_id 
FROM wp_postmeta pm WHERE pm.meta_key = '_order_total' and pm.meta_value > 0) 
and pm.meta_key = '_customer_user') as ou
INNER JOIN 
(SELECT pm.post_id AS order_id, pm.meta_value AS order_total 
FROM wp_postmeta pm WHERE pm.meta_key = '_order_total' and pm.meta_value > 0) as ot
ON
ou.order_id = ot.order_id)  as ou_final
group by ou_final.user_id) as payers
inner join wp_users us
